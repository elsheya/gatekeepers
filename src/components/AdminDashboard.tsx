import { useState } from 'react';
  ...
  export function AdminDashboard({ tickets, onDelete, onUpdate }: Props) {
    ...
    const handleSave = async () => {
      if (editingTicket) {
        const updatedTicket = {
          ...
          closedAt: editingTicket.status === 'Closed' ? new Date().toISOString() : null,
        };
    ...
    const getNotifiedColor = (notified: boolean) => {
      return notified ? 'bg-green-100 text-green-800 dark:bg-green-700 dark:text-green-100' : 'bg-red-100 text-red-800 dark:bg-red-700 dark:text-red-100';
    };
            ...
            {editingTicket && (
              <div className="space-y-6 py-4">
                ...
                <div className="space-y-2">
                  ...
                  >
                    <SelectTrigger className={cn(editingTicket.notified ? 'bg-green-100 text-green-800 dark:bg-green-700 dark:text-green-100' : 'bg-red-100 text-red-800 dark:bg-red-700 dark:text-red-100')}>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
